...
const char* flujo = "http://riegoapi.somee.com/api/historicoflujoes"; //para los post de flujo
...
// verificar si se encendio bomba desde internet
bool obtenerstatusbomba()
   {
    bool estado=false;
    if(WiFi.status()== WL_CONNECTED){
              
          sensorReadings = httpGETRequest(bomba);
          ...
          JSONVar value = myObject[0][1]; //o en la myObject[0][1]
          estado=bool(value);
    }
...
void Actualiza_Bomba (int valor)
{
...
void loop() {
  ...
  if (humedad < 30 || !obtenerstatusbomba())          // Si la humedad es menor a 30%, activa bomba por 5 segundos.
  {                          
